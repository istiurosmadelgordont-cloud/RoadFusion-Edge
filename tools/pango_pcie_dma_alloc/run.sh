...
# Stop immediately if a build, directory change, or module load fails.
set -e

# Resolve paths before sudo, while HOME still belongs to the SSH login user.
script_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
if [ "$(id -u)" -ne 0 ]; then
	exec sudo env DISPLAY="${DISPLAY:-}" \
