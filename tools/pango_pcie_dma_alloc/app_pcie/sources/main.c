...
SCREEN_INFO GuiScreen[] = 
{
	...
	{31, 	NULL, 						ENTRY,	NOTEBOOK_FRAME_FIXED_INDEX_1, ent_w, 0, ex1, ey1+es1*2,TRUE,NULL,	NULL, 	  "128"},
...
static void dma_oper_fun(DMA_AUTO *dma_auto, DMA_OPERATION *dma_oper)
{
	...
	dma_oper->current_len = dma_auto->start / 4;
}
...
void dma_auto_process(DMA_AUTO *dma_auto, DMA_OPERATION *dma_oper)
{
	...
	{
		dma_auto->step_add_cnt = (temp_step == 0)? 0: (((temp_end - temp_start + temp_step - 1)/temp_step));
